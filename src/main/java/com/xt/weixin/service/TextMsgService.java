...
//		
	 ...
	 * @return
	 */
	public PictureTextResponseVO handlePictureTextUnheard(WeixinVO requestVO){
		...
		Date now = new Date();
		responseVO.setCreateTime(now.getTime());
		List<TextMsg> msgs = null;
		try {
			List<PictureTextResponseVO.item> list =  new ArrayList<PictureTextResponseVO.item>();
			msgs = textMsgDao.find("闻所未闻");
			if(CollectionUtils.isNotEmpty(msgs)){
				responseVO.setArticleCount(msgs.size());
				for (TextMsg textMsg : msgs) {
					PictureTextResponseVO.item i1 = responseVO.new item();
					i1.setTitle(textMsg.getTitle());
					i1.setDescription(textMsg.getDescription());
					i1.setPicUrl(textMsg.getPicUrl());
					i1.setUrl(textMsg.getUrl());
					list.add(i1);
				}
			}
	 ...
	 * @return
	 */
	public PictureTextResponseVO handlePictureTextPrivileges(WeixinVO requestVO){
