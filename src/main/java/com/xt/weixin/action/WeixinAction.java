...
//	private UserService userService;
	...
	public void weixinSubscriptionDispatch() throws SQLException {
		   ...
		   if( weixinModel instanceof EventRequestVO ){
		    ...
		    if("CLICK".equals(eventVo.getEvent())&&"CONTACT".equals(eventVo.getEventKey())){
		    	...
		    	//订阅号统计
				subscriptionCountService.doSubscriptionCount("联系我们","享一代",eventVo.getFromUserName());
				//微信返回页面显示
				...
				//订阅号统计
				subscriptionCountService.doSubscriptionCount("闻所未闻","赏一代",eventVo.getFromUserName());
				//微信返回页面显示
				...
				//订阅号统计
				subscriptionCountService.doSubscriptionCount("尊享礼遇","赏一代",eventVo.getFromUserName());
				//微信返回页面显示
				...
				//订阅号统计
				subscriptionCountService.doSubscriptionCount("如鱼得水","赏一代",eventVo.getFromUserName());
				//微信返回页面显示
				...
				//订阅号统计
				subscriptionCountService.doSubscriptionCount("一步到位","赏一代",eventVo.getFromUserName());
				//微信返回页面显示
				...
				//订阅号统计
				subscriptionCountService.doSubscriptionCount("反馈专区","享一代",eventVo.getFromUserName());
				//微信返回页面显示
				...
				//订阅号统计
				subscriptionCountService.doSubscriptionCount("人气新星","居一代",eventVo.getFromUserName());
				//微信返回页面显示
			...
			if(StringUtils.equals(content, "陈奕迅")){
				//订阅号统计
				subscriptionCountService.doSubscriptionCount("尊享礼遇","赏一代",textVO.getFromUserName());
				//微信返回页面显示
				Object responseVO = textMsgService.handlePictureTextPrivileges(textVO);
				write(XmlUtils.writeXml(responseVO));
			}else{
				//订阅号统计
				subscriptionCountService.doSubscriptionCount("人气新星","居一代",textVO.getFromUserName());
				//微信返回页面显示
